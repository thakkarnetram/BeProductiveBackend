...
exports.signup = async (req, res) => {
  try {
    ...
    await sendEmails.verifyEmail(email, token);
...
exports.verifyEmail = async (req, res) => {
  ...
  if (!(email && token)) {
    res.status(401).json({ message: "Email and token not found" });
  }
  // verify jwt
  try {
...
exports.resetPasswordLink = async (req, res) => {
  try {
    ...
    const link = `http://localhost:8082/auth/api/v1/reset/${user._id}/${user.token}`;
